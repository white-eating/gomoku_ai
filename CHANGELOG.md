...
- `gui.py`：在第二周棋盘窗口（仅绘制网格 + hover 预览）基础上扩展为完整对弈界面，固定窗口 840×620（左侧棋盘区 + 右侧控制面板）
  - 新增右侧控制面板：标题、模式、当前回合（带状态圆点）、对局状态、AI 搜索统计（完成深度 / 搜索节点数 / 耗时 / 置换表命中率）、按钮区
  - 新增人机对弈完整闭环：鼠标点击交叉点落子（`_handle_click`）→ AI 迭代加深搜索回应（`_do_ai_move`），AI 思考时屏蔽棋盘误触
  - 新增 AI 自对弈模式（`_start_self_play`）：黑白双方均由 `MinimaxPlayer` 控制自动交替落子，支持暂停 / 继续 / 停止
  - 新增井字棋绘制（`_draw_ttt_piece`）：# 字格 + X(蓝) / O(红)，与五子棋共用同一套界面，可一键切换棋种（`_toggle_mode`）
  - 新增悔棋（`_handle_undo`，调用 `Game.undo(2)`）、重新开始（`_handle_new_game`）、置换表开关
  - 新增胜负弹窗（`_draw_popup`）：半透明遮罩 + 结果提示 + “再来一局”按钮，区分人机 / 自对弈、五子棋 / 井字棋的文案
  - 快捷键：`U` 悔棋、`N` 重新开始、`M` 切换模式、`S` 自对弈、`空格` 暂停 / 继续、`Q` 退出

## 已实现功能

- 完整 Pygame 图形界面：鼠标落子、AI 响应、悔棋、新游戏、模式切换、胜负弹窗
- AI 自对弈（黑 vs 白），可暂停 / 继续 / 停止
- 右侧面板实时显示 AI 搜索统计与置换表命中率
- 五子棋 / 井字棋共用界面，一键切换
